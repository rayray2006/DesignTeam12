...
import numpy as np
import random
from audio_utils_offline import *  # Import functions from offline utils
import time
...
CSV_PATH = RESULTS_DIR / "transcription_results_offine.csv"
EXCEL_PATH = RESULTS_DIR / "transcription_results_offline.xlsx"
LOG_PATH = RESULTS_DIR / "transcription_log_offline"
".txt"
...
# Loop to process backgrounds, gender, keyword, and volume combinations
for bg in BACKGROUNDS:
        ...
        for run_idx in range(N_RUNS):
            ...
            for keyword in KEYWORDS:
                for volume in VOLUMES:
                    ...
                    if detected_tool == keyword:
                        print(f"  → Detected: {detected_tool} | Confidence: {tool_conf:.2f}")
                    else:
                        print(f"  → Not Detected | Transcription: {transcription}")
    # Save raw data
    ...
    for keyword in KEYWORDS:
        ...
        plt.tight_layout()
        plt.savefig(PLOTS_DIR / f"{keyword}_accuracy_comparison_offline.png")
        plt.close()
    ...
    for keyword in df["keyword"].unique():
        ...
        plt.tight_layout()
        plt.savefig(PLOTS_DIR / f"{keyword}_detection_rate_summary_offline.png")
        plt.close()
    ...
    ax.set_title("Wakeword (ASTRA) Accuracy after 40 Runs with Offline Model")
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / "wakeword_detection_summary_offline.png")
    plt.close()
