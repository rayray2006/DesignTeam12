...
def get_hand_coords(color_frame, depth_frame):
    ...
    # mirror image before converting to grayscale
    color_image_flipped = cv2.flip(color_image, 1)

    # internal grayscale conversion so mp tracks contour not color
    grayscale_image = cv2.cvtColor(color_image_flipped, cv2.COLOR_BGR2GRAY)
    # convert grayscale to rgb format (same image)
    ...
    results = hands.process(grayscale_3channel)
